...
def verificarNome(nome: str):
    """
    Função para verificar se o nome fornecido possui apenas letras
    ...
    naoValidado = True
    while naoValidado:
        # Removendo todos os espaços encontrados na string associada à variável nome
        nome = nome.replace(' ', '')
        if nome.isalpha():
            naoValidado = False
        else:
...
def verificarTelefone(telefone: str):
    """
    Função para verificar se o telefone fornecido é númerico e se possui um dos seguintes formatos:
    +55 99 98888-8888, +55 99 8888-8888, 99 98888-8888, 99 8888-8888 (igmorando os espaços)
    ...
    while naoValidado:
        ...
        if telefone.startswith('+'):
            telefone = telefone[1:]
        if ((len(telefone) == 10 or len(telefone) == 11 or len(telefone) == 13 or len(telefone) == 14)
                and telefone.isnumeric()):
            naoValidado = False
        else:
            telefone = input('Número inválido, digite novamente: ')
            continue
    return telefone

def verificarEndereco(endereco: str):
    """
    Função para validar o endereco fornecido
    ...
    naoValidado = True
    while naoValidado:
        if (len(endereco) == 0) and not endereco.isalnum():
